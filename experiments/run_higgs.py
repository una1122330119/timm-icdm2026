#!/usr/bin/env python3
"""Higgs Twitter only — very large retweet network (n≈457K)."""
...
OUT = os.path.join(os.path.dirname(__file__), "..", "real_higgs.json")
data_dir = os.path.join(os.path.dirname(__file__), "..", "data")

print("Loading Higgs Twitter...")
g = load_dataset("higgs", data_dir=data_dir)
deg = g.m / max(g.n, 1)
beta = 1.0
# Target branching factor: subcritical b=0.7 prevents cascade saturation.
# For very sparse graphs (deg < 1.5), use α=0.5 → b ≈ deg·0.39.
if deg > 1.5:
    target_b = 0.7
    alpha = -beta * np.log(1.0 - target_b / deg)
else:
    alpha = 0.5  # sparse graph: moderate α, accept lower coverage
...
for k in K:
    ...
    for Cls, name in [
        ...
        results.append({"ds":"Higgs","algo":name,"k":k,"spread":round(sp,1),
